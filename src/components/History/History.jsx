...
const History = () => {
    ...
    return (
        <div className="max-w-7xl mx-auto p-8">
            <h3 className="text-3xl font-bold inter-600 text-center mb-5">
...

            {
                data.map((item, index) => (
                    <div key={index}>
                        <h1 className="text-xl text-gray-500 text-center mb-4">{item?.title}</h1>

                        {/* Pre-Independence Era */}
                        <h2 className="text-xl font-semibold mb-2 text-teal-500">Pre-Independence Era</h2>
                        <p className="mb-4 text-gray-500">
                            <strong className="text-gray-700">British Colonial Influence:</strong> {item?.pre_independence_era?.british_colonial_influence}
                        </p>
                        <p className="mb-4 text-gray-500">
                            <strong className="text-gray-700">Rural Industries:</strong> {item?.pre_independence_era?.rural_industries}
                        </p>

                        {/* Post-Independence Era */}
                        <h2 className="text-xl font-semibold mb-2 text-teal-500">Post-Independence Era</h2>
                        <p className="mb-4 text-gray-500">
                            <strong className="text-gray-700">Government Support:</strong> {item?.post_independence_era?.government_support}
                        </p>
                        <p className="mb-4 text-gray-500">
                            <strong className="text-gray-700">Global Market:</strong> {item?.post_independence_era?.global_market}
                        </p>
                        <p className="mb-4 text-gray-500">
                            <strong className="text-gray-700">Challenges and Opportunities:</strong> {item?.post_independence_era?.challenges_and_opportunities}
                        </p>
